import {
  collection, doc, getDocs, getDoc,
  query, updateDoc, where,
} from "firebase/firestore";
import { db } from "../../../FirebaseConfig";
import StorageService from "./StorageService";
import { Booking } from "./bookingService";
import { Chalet } from "./chaletService";
import { addNotification } from "./notificationService";

const isOwner = async () => {
  const user = await StorageService.getUser();
  return user?.role === "owner";
};

export const getOwnerPendingBookings = async (): Promise<Booking[]> => {
  if (!(await isOwner())) return [];
  const q = query(collection(db, "bookings"), where("status", "==", "pending"));
  const snapshot = await getDocs(q);
  return snapshot.docs.map((d) => ({ id: d.id, ...d.data() })) as Booking[];
};

export const confirmBooking = async (bookingId: string): Promise<void> => {
  await updateDoc(doc(db, "bookings", bookingId), { status: "confirmed" });

  const snap = await getDoc(doc(db, "bookings", bookingId));
  const booking = snap.data() as Booking;
  await addNotification(
    booking.userId,
    `تم قبول حجزك لشاليه "${booking.chaletName}"`,
    "booking_approved"
  );
};

export const rejectBooking = async (bookingId: string): Promise<void> => {
  await updateDoc(doc(db, "bookings", bookingId), { status: "rejected" });

  const snap = await getDoc(doc(db, "bookings", bookingId));
  const booking = snap.data() as Booking;
  await addNotification(
    booking.userId,
    `تم رفض حجزك لشاليه "${booking.chaletName}"`,
    "booking_rejected"
  );
};

export const getPendingChalets = async (): Promise<Chalet[]> => {
  if (!(await isOwner())) return [];
  const q = query(collection(db, "chalets"), where("approvalStatus", "==", "pending"));
  const snapshot = await getDocs(q);
  return snapshot.docs.map((d) => ({ id: d.id, ...d.data() })) as Chalet[];
};

export const approveChalet = async (chaletId: string): Promise<void> => {
  await updateDoc(doc(db, "chalets", chaletId), {
    approvalStatus: "approved",
    status: "available",
  });

  const snap = await getDoc(doc(db, "chalets", chaletId));
  const chalet = snap.data() as Chalet;
  if (chalet.ownerId) {
    await addNotification(
      chalet.ownerId,
      `تم قبول شاليهك "${chalet.name}" وهو الآن متاح للحجز`,
      "chalet_approved"
    );
  }
};

export const rejectChalet = async (chaletId: string): Promise<void> => {
  await updateDoc(doc(db, "chalets", chaletId), { approvalStatus: "rejected" });

  const snap = await getDoc(doc(db, "chalets", chaletId));
  const chalet = snap.data() as Chalet;
  if (chalet.ownerId) {
    await addNotification(
      chalet.ownerId,
      `تم رفض شاليهك "${chalet.name}" `,
      "chalet_rejected"
    );
  }
};