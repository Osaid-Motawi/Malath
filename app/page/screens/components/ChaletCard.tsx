...
import { Dimensions, Image, NativeScrollEvent, NativeSyntheticEvent,
...
function ChaletCard({ chalet, onPress, cardWidth }: Props) {
    ...
    const imgWidth = cardWidth ?? CARD_WIDTH;
    const imgHeight = cardWidth ? imgWidth * 0.75 : 205;
    ...
    function handleScroll(e: NativeSyntheticEvent<NativeScrollEvent>) {
        setActiveIndex(Math.round(e.nativeEvent.contentOffset.x / imgWidth));
    }
                    ...
                    >
                        {images.map((uri, i) => (
                            <Image
                                key={i}
                                source={{ uri }}
                                style={{ width: imgWidth, height: imgHeight, backgroundColor: "#e5e7eb" }}
                                resizeMode="cover"
                            />
                        ))}
                    </ScrollView>
                ) : (
                    <View style={{ width: imgWidth, height: imgHeight, backgroundColor: "#e5e7eb" }} />
                )}
...
const s = StyleSheet.create({
    ...
    dots: { position: "absolute", bottom: 9, left: 0, right: 0,
